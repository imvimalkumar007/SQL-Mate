...
- **Content Security Policy.** A strict CSP is set in `tauri.conf.json`: `default-src 'self'`, `script-src 'self'`, `font-src 'self'` (no CDN fonts), `img-src 'self' data:` (inline SVG icons only). `connect-src` allows `'self' https:` — the `https:` wildcard is deliberately broad because the LLM provider base URL is user-configured at runtime; the effective destination is constrained to what the user entered in provider settings, which is auditable in the request log. This CSP prevents any inline script injection, any external stylesheet load, and any unanticipated outbound fetch target that would be visible only to the WebView.
...
9. Inspecting `src/widget.html` and `src/Widget.tsx` and confirming no third-party script tags, no `<link rel="stylesheet" href="https://...">`, no `fetch()` to anywhere except the existing Tauri command surface. The Content Security Policy in `tauri.conf.json` enforces this at the WebView level — any violation is blocked by the browser engine, not just absent from the code.
