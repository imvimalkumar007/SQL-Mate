...
4. SQL renders with syntax highlighting and a "Copy" button. The
   widget does not run SQL — same SECURITY_MODEL T2 stance as the
   main window.
5. Footer shows the latency (with a speed icon), and "validated" or
   "rejected" depending on Layer 2's verdict.
6. Last position, last question, last SQL, and last validation status
